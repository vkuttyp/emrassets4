...
export const useCarsStore = defineStore({
  ...
  getters: {
    subordinateCarRequests: (state) =>
      Object.values(state.subordinates).filter((s) => s.carRequests?.length),
    votingIncomple: (state) =>
      Object.values(state.carVotingPendingList).filter(
        (s) => s.totalVotesCount > s.totalVotesObtained
      ),
  ...
  actions: {
    ...
    async carManagerResponse(managerResponse) {
      this.carRequest = { loading: true }
      return await fetchWrapper
        .post(`${baseUrl}/ResponseCar`, managerResponse)
        .catch((error) => (this.carRequest = { error }))
    },
    ...
    async carFinalDecision(decision) {
      this.saving = { loading: true }
      return await fetchWrapper
        .post(`${baseUrl}/CarFinalDecision`, decision)
        .catch((error) => (this.saving = { error }))
    },
